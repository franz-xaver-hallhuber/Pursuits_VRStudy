...
public class PupilCalibMarker : MonoBehaviour {
	...
	void Update () {
		if (Input.GetKeyDown (KeyCode.C))
        {
            ...
            GameObject.Find("CalibrationCanvas").GetComponent<Canvas>().enabled = true;
        }

        if (Input.GetKeyDown (KeyCode.S))
        {
            PupilGazeTracker.Instance.StopCalibration();
            GameObject.Find("CalibrationCanvas").GetComponent<Canvas>().enabled = false;
        }
			
        if (Input.GetKeyDown (KeyCode.V))
        {
            if(_started) PupilGazeTracker.Instance.StopCalibration();
            bool state = GameObject.Find("CalibrationCanvas").GetComponent<Canvas>().enabled;
            GameObject.Find("CalibrationCanvas").GetComponent<Canvas>().enabled = !state;
        }
